...
CRON.prototype={
        ...
        this.api.log(jobName,context.currentPage,context.pageSize,function(data){
            ...
            for (var i=0;i<records.length;i++){
                html+="<tr data-name='"+records[i]["JobName"]+"'><td>"+records[i]['JobName']+"</td>"+
                    ...
                    "<td>"+records[i]['Err']+"</td>"+
                    "<td><button class='btn btn-info JS-output-view' data-output='"+context.htmlEncode(records[i]['Output'],records[i]['Command'])+"'>查看</button></td>"+
                    "</tr>";
            }
            $('.JS-job-list').html(html)
    ...
    htmlEncode:function(str,command){
